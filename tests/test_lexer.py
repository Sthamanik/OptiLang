...
def get_types(source: str) -> list[str]:
...
class TestNumbers:
    ...
    def test_integer_is_int_not_float(self) -> None:
    ...
    def test_float_is_float_not_int(self) -> None:
        token = first_token("3.14")
        assert isinstance(token.value, float)

    def test_float_starting_with_zero(self) -> None:
        token = first_token("0.5")
        assert abs(token.value - 0.5) < 1e-9

    def test_multiple_decimal_points_raises_error(self) -> None:
        with pytest.raises(LexerError):
            tokenize("3.1.4")

    def test_float_ending_with_decimal_raises_error(self) -> None:
        with pytest.raises(LexerError):
            tokenize("3.")
    ...
    def test_number_line_number(self) -> None:
        token = first_token("42")
        assert token.line == 1

    def test_number_column_number(self) -> None:
        token = first_token("42")
...
class TestStrings:
    ...
    def test_double_quoted_string_type(self) -> None:
        token = first_token('"hello"')
        assert token.type == TokenType.STRING

    def test_double_quoted_string_value(self) -> None:
        token = first_token('"hello"')
        assert token.value == "hello"

    def test_single_quoted_string_type(self) -> None:
        token = first_token("'hello'")
        assert token.type == TokenType.STRING

    def test_single_quoted_string_value(self) -> None:
        token = first_token("'hello'")
        assert token.value == "hello"
    ...
    def test_escape_double_quote_inside_double_quoted(self) -> None:
    ...
    def test_unterminated_single_quote_raises_error(self) -> None:
        with pytest.raises(LexerError):
            tokenize("'unterminated")
...
class TestBooleanAndNone:
    ...
    def test_true_lowercase_is_identifier(self) -> None:
        token = first_token("true")
        assert token.type == TokenType.IDENTIFIER

    def test_false_lowercase_is_identifier(self) -> None:
        token = first_token("false")
        assert token.type == TokenType.IDENTIFIER
...
class TestKeywords:
    ...
    def test_keyword_token_type(self, keyword: str, expected_type: str) -> None:
    ...
    def test_keyword_with_numbers_is_identifier(self) -> None:
...
class TestIdentifiers:
    ...
    def test_multi_char_identifier(self) -> None:
    ...
    def test_all_uppercase_identifier(self) -> None:
        token = first_token("TOTAL")
        assert token.type == TokenType.IDENTIFIER
        assert token.value == "TOTAL"
...
class TestSingleCharOperators:
    ...
    def test_single_char_operator(self, source: str, expected_type: str) -> None:
...
class TestTwoCharOperators:
    ...
    def test_two_char_operator(self, source: str, expected_type: str) -> None:
        token = first_token(source)
        assert token.type == expected_type

    def test_eq_not_confused_with_assign(self) -> None:
        tokens = tokenize("==")
        assert tokens[0].type == TokenType.EQ

    def test_assign_not_confused_with_eq(self) -> None:
        tokens = tokenize("=")
        assert tokens[0].type == TokenType.ASSIGN

    def test_power_not_confused_with_multiply(self) -> None:
        tokens = tokenize("**")
        assert tokens[0].type == TokenType.POWER

    def test_floor_divide_not_confused_with_divide(self) -> None:
        tokens = tokenize("//x")
        assert tokens[0].type == TokenType.FLOOR_DIVIDE

    def test_plus_assign_not_confused_with_plus(self) -> None:
        tokens = tokenize("+=")
        assert tokens[0].type == TokenType.PLUS_ASSIGN

    def test_ne_token_value(self) -> None:
        token = first_token("!=")
        assert token.value == "!="
...
class TestIndentation:
    ...
    def test_indent_produced_on_block(self) -> None:
        source = "if True:\n    x = 1"
        types = get_types(source)
        assert TokenType.INDENT in types

    def test_dedent_produced_after_block(self) -> None:
        source = "if True:\n    x = 1\ny = 2"
        types = get_types(source)
        assert TokenType.DEDENT in types

    def test_nested_indent_produces_two_indents(self) -> None:
        source = "if True:\n    if True:\n        x = 1"
        types = get_types(source)
        assert types.count(TokenType.INDENT) == 2

    def test_nested_dedent_produces_two_dedents(self) -> None:
        source = "if True:\n    if True:\n        x = 1\ny = 2"
        types = get_types(source)
        assert types.count(TokenType.DEDENT) == 2

    def test_invalid_indentation_not_multiple_of_4_raises_error(self) -> None:
        source = "if True:\n   x = 1"  # 3 spaces
        with pytest.raises(LexerError):
            tokenize(source)

    def test_two_space_indent_raises_error(self) -> None:
        source = "if True:\n  x = 1"  # 2 spaces
        with pytest.raises(LexerError):
            tokenize(source)

    def test_valid_8_space_double_nested_indent(self) -> None:
        source = "if True:\n    if True:\n        x = 1"
        tokens = tokenize(source)
        assert tokens is not None

    def test_tab_treated_as_indent(self) -> None:
        source = "if True:\n\tx = 1"
        types = get_types(source)
        assert TokenType.INDENT in types

    def test_no_indent_at_top_level(self) -> None:
        source = "x = 1\ny = 2"
        types = get_types(source)
...
class TestComments:
    ...
    def test_comment_only_produces_eof(self) -> None:
        tokens = tokenize("# this is a comment")
        assert tokens[-1].type == TokenType.EOF
        assert len(tokens) == 1

    def test_comment_after_code_ignored(self) -> None:
        types = get_types("x = 5 # assign x")
        assert TokenType.NUMBER in types
        assert TokenType.ASSIGN in types

    def test_comment_does_not_produce_tokens(self) -> None:
        source = "x = 1\n# comment\ny = 2"
        tokens = tokenize(source)
...
class TestNewlines:
    ...
    def test_newline_token_produced(self) -> None:
        source = "x = 1\ny = 2"
        types = get_types(source)
        assert TokenType.NEWLINE in types

    def test_multiple_blank_lines_produce_single_newline(self) -> None:
    ...
    def test_newline_between_statements(self) -> None:
        source = "x = 1\ny = 2"
        types = get_types(source)
...
class TestTokenMetadata:
    ...
    def test_first_token_is_line_1(self) -> None:
        token = first_token("x")
        assert token.line == 1

    def test_first_token_is_column_1(self) -> None:
        token = first_token("x")
        assert token.column == 1
    ...
    def test_token_column_after_spaces(self) -> None:
    ...
    def test_error_contains_column_number(self) -> None:
        with pytest.raises(LexerError) as exc_info:
            tokenize('"unterminated')
...
class TestIllegalCharacters:
    ...
    def test_at_sign_raises_lexer_error(self) -> None:
        with pytest.raises(LexerError):
            tokenize("@")
    ...
    def test_hash_is_not_illegal_it_is_comment(self) -> None:
        """# starts a comment, should not raise error."""
        tokens = tokenize("# comment")
        assert tokens[-1].type == TokenType.EOF

    def test_exclamation_alone_raises_error(self) -> None:
...
class TestEOF:
    ...
    def test_eof_value_is_none(self) -> None:
        tokens = tokenize("x = 1")
...
class TestCompleteExpressions:
    ...
    def test_for_loop_token_sequence(self) -> None:
    ...
    def test_if_statement_token_sequence(self) -> None:
        types = get_types("if x == 5:")
        assert TokenType.IF in types
        assert TokenType.EQ in types
        assert TokenType.COLON in types

    def test_def_statement_token_sequence(self) -> None:
    ...
    def test_augmented_assignment_sequence(self) -> None:
        types = get_types("x += 1")
        assert types[0] == TokenType.IDENTIFIER
        assert types[1] == TokenType.PLUS_ASSIGN
        assert types[2] == TokenType.NUMBER

    def test_list_literal_sequence(self) -> None:
        types = get_types("[1, 2, 3]")
        assert types[0] == TokenType.LBRACKET
        assert TokenType.COMMA in types
        assert TokenType.RBRACKET in types

    def test_dict_literal_sequence(self) -> None:
