...
def make_suggestion(severity: str):
    """Return a minimal mock Suggestion-like object."""
    class _S:
        pass
    s = _S()
    s.severity = severity
...
class TestScoreReport:
    ...
    def test_to_dict_has_required_keys(self):
        r = ScoreReport(
            ...
            breakdown={"severity_penalty": 0.0, "complexity_penalty": 5.0,
                       "performance_penalty": 2.0, "memory_penalty": 1.0},
        ...
        d = r.to_dict()
        assert set(d.keys()) == {
            "score", "grade", "complexity_class", "breakdown",
            "max_execution_count", "lines_profiled", "baseline_time_ms",
        }
    ...
    def test_to_dict_breakdown_rounded(self):
        r = ScoreReport(
            score=80.0, grade="Good", complexity_class="O(n)",
...
class TestSeverityPenalty:
    ...
    def test_no_suggestions_returns_zero(self):
        s = Scorer(make_profiling())
        assert s._severity_penalty() == 0.0

    def test_single_high_suggestion(self):
        s = Scorer(
            make_profiling(),
    ...
    def test_single_medium_suggestion(self):
        s = Scorer(
            make_profiling(),
    ...
    def test_single_low_suggestion(self):
        s = Scorer(
            make_profiling(),
    ...
    def test_mixed_suggestions(self):
        suggestions = [
    ...
    def test_short_program_uses_floor_of_10(self):
        """Programs shorter than 10 lines use 10 as the normaliser floor."""
        suggestions = [make_suggestion("high")]
        s_short  = Scorer(make_profiling(), suggestions=suggestions, total_source_lines=3)
        s_medium = Scorer(make_profiling(), suggestions=suggestions, total_source_lines=10)
        # Both should produce the same result because floor is 10
        assert s_short._severity_penalty() == pytest.approx(s_medium._severity_penalty())
...
class TestComplexityDetection:
    ...
    def test_empty_profiling_returns_O1(self):
        s = Scorer(make_profiling(line_counts=[0]))
        assert s._detect_complexity() == "O(1)"

    def test_single_execution_returns_O1(self):
    ...
    def test_nested_loop_O_n2(self):
    ...
    def test_triple_nested_O_n3(self):
    ...
    def test_deep_nesting_O_nk(self):
...
class TestComplexityPenalty:
    ...
    def test_O1_penalty_is_zero(self):
        s = Scorer(make_profiling(line_counts=[1]))
        assert s._complexity_penalty() == 0.0

    def test_On_penalty_is_zero(self):
        # Linear loop — header + body both run 100 times
        # avg = (1+100+100)/3 = 67, avg×1.5 = 100.5
        # 100 >= 90 (peak_threshold) AND 100 > 100.5? NO → nesting_level=0 → O(n) → penalty=0
        s = Scorer(make_profiling(line_counts=[1, 100, 100]))
        assert s._complexity_penalty() == 0.0

    def test_On2_penalty(self):
        s = Scorer(make_profiling(line_counts=[101, 10100, 10000]))
        assert s._complexity_penalty() == pytest.approx(
            COMPLEXITY_PENALTIES["O(n²)"]
        )

    def test_complexity_penalty_clamped(self):
        # O(2^n) raw penalty is 35, but MAX is 30
        s = Scorer(make_profiling(line_counts=[1]))
        # Manually call with a complexity that exceeds the cap
...
class TestPerformancePenalty:
    ...
    def test_at_baseline_no_penalty(self):
    ...
    def test_twice_baseline_gives_penalty(self):
        # ratio = 2, raw = (2-1)×3 = 3
    ...
    def test_below_baseline_penalty_is_zero(self):
...
class TestMemoryPenalty:
    ...
    def test_no_memory_vars_returns_zero(self):
    ...
    def test_all_lines_exceed_threshold(self):
    ...
    def test_half_lines_exceed_threshold(self):
        threshold = HIGH_VAR_THRESHOLD + 1
        p = make_profiling(
            line_counts=[1, 1, 1, 1],
...
class TestCalculate:
    ...
    def test_perfect_program_high_score(self):
    ...
    def test_nested_loop_lower_score(self):
    ...
    def test_grade_poor_on_bad_code(self):
    ...
    def test_score_is_clamped_below_100(self):
...
class TestCalculateScoreFunction:
    ...
    def test_matches_scorer_directly(self):
        p = make_profiling(line_counts=[1, 100, 100])
        suggestions = [make_suggestion("medium")]
...
class TestEdgeCases:
    ...
    def test_empty_line_stats(self):
        p = {
            "line_stats": {},
            "function_stats": {},
            "total_time_ms": 0.0,
    ...
    def test_very_large_execution_count(self):
        # Should not crash or overflow
        p = make_profiling(line_counts=[1, 10, 1_000_000])
        report = calculate_score(p)
        assert 0.0 <= report.score <= 100.0

    def test_total_source_lines_zero_handled(self):
        # Should not divide by zero
        p = make_profiling(line_counts=[1])
        report = calculate_score(p, total_source_lines=0)
        assert report.score >= 0.0

    def test_all_lines_same_count_linear(self):
