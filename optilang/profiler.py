...
class ProfilingData:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            'line_stats': {
                line: stats.to_dict() 
                for line, stats in sorted(self.line_stats.items())
            },
            'function_stats': {
                name: stats.to_dict() 
    ...
    def stop(self) -> None:
        ...
        if self.data.start_time is not None:
            ...
            self.data.total_execution_time_ms = (
                (self.data.end_time - self.data.start_time) * 1000
            )
        
        # Calculate total lines executed
        self.data.total_lines_executed = sum(
            stats.execution_count 
            for stats in self.data.line_stats.values()
        )
...
if __name__ == "__main__":
    ...
    for line, stats in sorted(data.line_stats.items()):
        print(f"  Line {line}: {stats.execution_count} executions, "
              f"{stats.total_time_ms:.3f}ms total")
    print("\nFunction stats:")
    for name, stats in data.function_stats.items():
        print(f"  {name}: {stats.call_count} calls, "
              f"{stats.total_time_ms:.3f}ms total")